...
uint16_t g_relay_port = DEF_RELAY_CLI_PORT;

char g_server_ipstr[HOST_IP_LEN + 1] = {0};
uint32_t g_server_ip = 0;

uint16_t g_local_port = DEF_CLIENT_PORT;

char g_username[MAX_USERNAME_LEN + 1] = {0};
char g_password[MAX_PASSWD_LEN + 1] = {0};
...
static int cmd_parser(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "s:l:r:d:p:u:w:m:e:h")) != -1) {
		switch (opt) {
		case 's':
			strncpy(g_server_ipstr, optarg, HOST_IP_LEN);
			if (0 != util_str_to_ip(g_server_ipstr, &ipaddr))
			{
				printf("Invalid socks server %s\n", g_server_ipstr);
				Usage(argv[0]);
				return -1;
			}
			g_server_ip = htonl(((struct sockaddr_in *)&ipaddr)->sin_addr.s_addr);
		...
		case 'p':
			...
			break;
		case 'u':
			strncpy(g_username, optarg, MAX_USERNAME_LEN);
			printf("get option: user name %s\n", g_username);
			break;
		case 'w':
			strncpy(g_password, optarg, MAX_PASSWD_LEN);
			printf("get option: user name %s\n", g_password);
			break;
		case 'm':
                ...
                void* param3, void* param4)
{
    ...
    {
        g_RemoteServ = new CRemoteServer(g_relay_ip, g_relay_port);
