...
int CVpnRemote::send_client_iobusy_msg(bool isBusy)
{
    ...
    if (isBusy)
    {
        c2rhdr.sub_type = CLIENT_IO_BUSY;
    }
    else
    {
        c2rhdr.sub_type = CLIENT_IO_RESUME;
    }
    ...
    _LOG_INFO("client(0x%x/%u/fd%d) send client ioBusy(%d) msg to remote", 
        pConn->get_client_ipaddr(), pConn->get_client_port(), pConn->get_client_fd(), isBusy);
    return 0;
}
