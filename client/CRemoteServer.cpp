...
int CRemoteServer::build_authen_msg(char *buf)
{
    ...
    buf[0] = 0x01;
    buf[1] = strlen(g_username);
    strncpy(&buf[2], g_username, buf[1]);
    pos += buf[1] + 2;

    buf[pos] = strlen(g_password);
    strncpy(&buf[pos+1], g_password, buf[pos]);
    pos += buf[pos] + 1;

    return pos;
}

int CRemoteServer::parse_authen_result_msg(char *buf, int buf_len)
{
    if (buf[0] != 0x01)
    {
...
int CRemoteServer::parse_connect_result_msg(char *buf, int buf_len)
{
...
int CRemoteServer::send_auth_quest_msg()
{
    ...
    c2rhdr.client_ip = m_local_ipaddr;
    c2rhdr.server_port = 0;
    c2rhdr.client_port = g_local_port;
    c2rhdr.sub_type = C2R_AUTH;
    c2rhdr.reserved = 0;
...
int CRemoteServer::pdu_handle(char *pdu_buf, int pdu_len)
{
    ...
    else
    {
        ...
        {
            if (CLI_CONNECTING != pConn->get_client_status())
            {
